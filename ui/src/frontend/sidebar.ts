...
export class Sidebar implements m.ClassComponent<OptionalTraceImplAttrs> {
  ...
  view({attrs}: m.CVnode<OptionalTraceImplAttrs>) {
    ...
    const trace = attrs.trace ?? app.trace;
    const sidebar = trace?.sidebar ?? app.sidebar;
  ...
  private renderSection(
    ...
    switch (sectionId) {
      ...
      case 'convert_trace':
        ...
        break;
      case 'support':
        allItems.push(...getSupportGlobalItems(trace ?? app));
        if (trace !== undefined) {
    ...
    const menuItems = allItems
      ...
      .map((item) => this.renderItem(item, trace));
