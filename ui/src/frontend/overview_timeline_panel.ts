...
import { Span, TPTime, tpTimeToSeconds } from '../common/time';

import { OVERVIEW_TIMELINE_NON_VISIBLE_COLOR, SIDEBAR_WIDTH, TRACK_SHELL_WIDTH } from './css_constants';
...
export class OverviewTimelinePanel extends Panel {
    ...
    onupdate({ dom }: m.CVnodeDOM) {
        ...
        if (newWidth > 0) {
            // It may be zero when temporarily not visible
            this.width = newWidth;
        }
    ...
    oncreate(vnode: m.CVnodeDOM) {
        this.onupdate(vnode);
        (vnode.dom as HTMLElement).addEventListener('mousemove', this.boundOnMouseMove);
    }

    onremove({ dom }: m.CVnodeDOM) {
        (dom as HTMLElement).removeEventListener('mousemove', this.boundOnMouseMove);
    }
    ...
    renderCanvas(ctx: CanvasRenderingContext2D, size: PanelSize) {
        ...
        if (size.width > TRACK_SHELL_WIDTH && this.traceTime.duration > 0n) {
            ...
            const tickGen = new TickGenerator(this.traceTime, maxMajorTicks, globals.state.traceTime.start);

            // Set a background of color --perfetto-overview-background
            const perfettoOverviewBackground = getComputedStyle(document.documentElement).getPropertyValue(
                '--perfetto-overview-background'
            );
            // getPropertyValue returns output of" "actualColor" "
            // This cuts it down to "actualColor"
            const style = perfettoOverviewBackground.substring(2, perfettoOverviewBackground.length - 1) || 'white';
            ctx.fillStyle = style;
        ...
        ctx.fillRect(TRACK_SHELL_WIDTH - 1, headerHeight, vizStartPx - TRACK_SHELL_WIDTH, tracksHeight);
    ...
    private chooseCursor(x: number) {
        ...
        if (OverviewTimelinePanel.inBorderRange(x, startBound) || OverviewTimelinePanel.inBorderRange(x, endBound)) {
            return 'ew-resize';
    ...
    onDragStart(x: number) {
        ...
        if (OverviewTimelinePanel.inBorderRange(x, pixelBounds[0]) || OverviewTimelinePanel.inBorderRange(x, pixelBounds[1])) {
    ...
    private static extractBounds(timeScale: TimeScale): [number, number] {
        ...
        return [Math.floor(timeScale.hpTimeToPx(vizTime.start)), Math.ceil(timeScale.hpTimeToPx(vizTime.end))];
    }
